...
`/equities/bars/daily` は **1 営業日あたり約 4,200 行**（全銘柄）返る。
2026-09-05 に 2 営業日ぶんを取って 8,409 行だった（`/equities/master` は同時点で 4,236 銘柄）。

バックフィルの範囲を決めるときはこの数字から見積もる
（`docs/GO-LIVE.md` B3 に 2 年ぶんの換算がある）。
